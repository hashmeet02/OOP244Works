...
// Hashmeet Singh Saini	   July 30th			Modification to the checkOutPub method
// Hashmeet Singh Saini	   July 30th			Modification to the returnPub method
// Hashmeet Singh Saini	   July 30th			added some fool-proofing to the new publication method
// -----------------------------------------------------------
...
namespace sdds {
	...
	void LibApp::returnPub()
	{
		...
		if (selection) {
			...
			if (flag) {
				days = today - m_publication[location]->checkoutDate();
				if (days > 15) {
					penalty = 50 * (days - 15);
					cout << "Please pay $";
					cout.setf(ios::fixed);
					cout.precision(2); 
					cout<< penalty / 100 << " penalty for being " << days - 15 << " days late!" << endl;
					m_publication[location]->set(0);
	...
	{
		if (m_numLoadedPubs < SDDS_LIBRARY_CAPACITY) {
			bool flag = 0;
			int selection = 0;
			Publication* tempPub{};
