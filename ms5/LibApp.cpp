...
namespace sdds {
	...
	int LibApp::search(int searchType)
	{
		...
		if (selection == 0) {
			...
			for (int i = 0; i < m_numLoadedPubs; i++) {
				if (m_publication[i]->getRef() != 0 && m_publication[i]->type() == selection && m_publication[i]->operator==(searchTitle)) {
					switch (searchType)
					{
					case 1:
	...
	void LibApp::removePublication()
	{
		...
		if (flag) {
			m_publication[selection]->setRef(0);
			m_changed = true;
